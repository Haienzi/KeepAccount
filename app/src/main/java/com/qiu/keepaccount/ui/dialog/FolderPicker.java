...
import com.qiu.keepaccount.util.BackUpAndRestore;
 ...
 * @author 常兴
 */
public class FolderPicker extends Dialog implements OnItemClickListener,
        OnClickListener {
    ...
    public FolderPicker(Context context, OnClickListener mListener2, int themeId) {
        this(context, mListener2, themeId, false);
    }
    ...
    public FolderPicker(Context context, OnClickListener listener, int themeId,
        ...
        setTitle(acceptFiles ? "选择文件": "选择文件夹");
        setContentView(R.layout.folders);

        mRootSDCard = new File(BackUpAndRestore.getStoragePath(context,true));
        ...
        mPath = new Folder(BackUpAndRestore.getStoragePath(context,true));
        updateAdapter();
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position,
        ...
        if (mAcceptFiles) {
            ...
            if (item.isDirectory() && item.canRead() && item.canRead()) {
                mPath = item;
                updateAdapter();
                mFilePath = null;
            } else {
    ...
    class FolderAdapter extends BaseAdapter {
        ...
        public FolderAdapter() {
            ...
            mFolderUpLayers = new Drawable[]{
                    res.getDrawable(R.drawable.ic_folder_up_closed),
                    res.getDrawable(R.drawable.ic_folder_up_open),};
            mFolderLayers = new Drawable[]{
                    res.getDrawable(R.drawable.ic_folder_close),
                    res.getDrawable(R.drawable.ic_folder_open),};
    ...
    static class FolderTransitionDrawable extends LayerDrawable implements
            Runnable {
        ...
        protected boolean onStateChange(int[] state) {
            ...
            if (active != mActive) {
                mActive = active;
                // Log.d("FolderTransitionDrawable", "onStateChange " +
                // StateSet.dump(state) + " " + active);
                if (!active) {
                    unscheduleSelf(this);
                    if (mAlpha != 255) {
        ...
        public void draw(Canvas canvas) {
            ...
            if (mStartTime != 0) {
                ...
                if (done) {
                    ...
                    float normalized = (time - mStartTime)
